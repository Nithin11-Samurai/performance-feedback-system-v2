...
const getDirectReportsOf = asyncHandler(async (req, res) => {
  const reports = await userService.getDirectReports(
    req.user,
    req.params.userId
  );
...
const deactivateUser = asyncHandler(async (req, res) => {
  const updated = await userService.setActiveStatus(
    req.user,
    req.params.userId,
    false,
    getRequestMeta(req)
...
const deleteEmployee = asyncHandler(async (req, res) => {
  const deleted = await userService.deleteEmployee(
    req.user,
    req.params.userId,
    getRequestMeta(req)
  );
  ...
  res.json({
    success: true,
    message: 'Employee moved to Recently Deleted',
    data: { user: deleted },
  });
});
...
const restoreEmployee = asyncHandler(async (req, res) => {
  const restored = await userService.restoreEmployee(
    req.user,
    req.params.userId,
    getRequestMeta(req)
  );
...
// multipart/form-data, field: avatarFile
const uploadAvatar = asyncHandler(async (req, res) => {
  ...
  // Multer now uses memoryStorage(), so req.file.filename no longer exists.
  // Pass the complete file object to the service so it can upload the
  // buffer to Azure Blob Storage.
...
const bulkAssignManager = asyncHandler(async (req, res) => {
  ...
  res.json({
    success: true,
    message: `${updated.length} employee(s) reassigned`,
    data: { users: updated },
  });
});
...
const getTimeline = asyncHandler(async (req, res) => {
  const events = await timelineService.getEmployeeTimeline(
    req.user,
    req.params.userId
  );
...
const globalSearch = asyncHandler(async (req, res) => {
  const results = await userService.globalSearch(
    req.user,
    req.query.q
  );
